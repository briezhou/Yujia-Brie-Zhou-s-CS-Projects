...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        for variable in eliminationOrder:
            FactorList, new_fact = joinFactorsByVariable(FactorList, variable)
            if len(new_fact.unconditionedVariables()) > 1:
                vectorElimination = eliminate(new_fact, variable)
                FactorList.append(vectorElimination)
        Join = joinFactors(FactorList)
        Norm = normalize(Join)
        return Norm
...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        for position in self.allPositions:
            belief[position] *= self.getObservationProb(observation, PacmanPos, position, JailPos)
        self.beliefs = belief
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState: busters.GameState):
        ...
        """
        self.particles = []
        spaceSize = len(self.legalPositions)
        fullCycles = self.numParticles // spaceSize
        remainder = self.numParticles % spaceSize
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        for position in self.particles:
            observed = self.getObservationProb(observation, PacmanPos, position, JailPos)
            if position in Dist.keys():
                new_belief[position] += observed
            else:
                new_belief[position] = observed
        new_belief.normalize()
        if new_belief.total() > 0:
            self.particles = [new_belief.sample() for _ in range(len(self.particles))]
        else:
            self.initializeUniformly(gameState)
    ...
    def elapseTime(self, gameState):
        ...
        for pos in self.allPositions:
            ...
            for newPos, prob in newPosDist.items():
                new[newPos] += prob * old[pos]
        new.normalize()
        self.particles = [new.sample() for _ in range(len(self.particles))]
