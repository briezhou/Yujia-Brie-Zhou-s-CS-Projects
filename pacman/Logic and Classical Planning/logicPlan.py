...
def sentence1() -> Expr:
    ...
    thirdstatement = disjoin((~A),(~B),C)

    return conjoin([firststatement,secondstatement,thirdstatement])
...
def sentence3() -> Expr:
    ...
    """
    PacmanAlive0 = Expr('PacmanAlive_0')
    PacmanAlive1 = Expr('PacmanAlive_1')
    PacmanBorn0 = Expr('PacmanBorn_0')
    PacmanKilled0 = Expr('PacmanKilled_0')
    Sentence1 = PacmanAlive1 % (PacmanAlive0 & (~PacmanKilled0) | (~PacmanAlive0) & PacmanBorn0) 
    Sentence2 = ~(PacmanAlive0 & PacmanBorn0)
    Sentence3 = PacmanBorn0
...
def entails(premise: Expr, conclusion: Expr) -> bool:
    ...
    if not findModel(combined_expr):
        return True
    return False
...
def pacphysicsAxioms(t: int, all_coords: List[Tuple], non_outer_wall_coords: List[Tuple], walls_grid: List[List] = None, sensorModel: Callable = None, successorAxioms: Callable = None) -> Expr:
    ...
    for point in all_coords:
        wallHere = PropSymbolExpr(wall_str, point[0], point[1])
        pacHere = PropSymbolExpr(pacman_str, point[0], point[1], time=t)
        pacphysics_sentences.append(wallHere >> ~pacHere)

    pacphysics_sentences.append(exactlyOne([PropSymbolExpr(pacman_str, x, y, time=t) for (x, y) in non_outer_wall_coords]))

    possibleMoves = [PropSymbolExpr('North', time=t), PropSymbolExpr('South', time=t), 
...
def positionLogicPlan(problem) -> List:
    ...
    for t in range(50):
        ...
        possibleMoves = [PropSymbolExpr(action, time=t) for action in actions]
        KB.append(exactlyOne(possibleMoves))
        goalReached = PropSymbolExpr(pacman_str, xg, yg, time=t)
        goalModel = findModel(conjoin(KB + [goalReached]))
        if goalModel:
...
def foodLogicPlan(problem) -> List:
    ...
    for t in range(50):
        ...
        allEaten = []
        for (x, y) in food:
            currentPac = PropSymbolExpr(pacman_str, x, y, time=t)
            alreadyGone = ~PropSymbolExpr(food_str, x, y, time=t)
            foodNextStep = (currentPac | alreadyGone) % ~PropSymbolExpr(food_str, x, y, time=t + 1)
            KB.append(foodNextStep)
            allEaten.append(~PropSymbolExpr(food_str, x, y, time=t + 1))

        allFoodEaten = conjoin(allEaten)
        conjoined = conjoin(KB)
        goalModel = findModel(allFoodEaten & conjoined)
        if goalModel != False:
            return extractActionSequence(goalModel, actions)

        possibleMoves = [PropSymbolExpr(move, time=t) for move in actions]
        KB.append(exactlyOne(possibleMoves))
