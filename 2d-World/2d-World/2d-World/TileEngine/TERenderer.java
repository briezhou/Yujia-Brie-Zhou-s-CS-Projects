...
public class TERenderer {
    ...
    public void renderFrame(TETile[][] world, String block) {
        ...
        StdDraw.text((world.length) / 2 - 10, world[0].length + 1, block);
        LocalDateTime now = LocalDateTime.now();
        DateTimeFormatter formatter = DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss");
        String formattedDateTime = "Time Right Now:  " + now.format(formatter);
        StdDraw.text((world.length) / 2 + 10, world[0].length + 1, formattedDateTime);
        StdDraw.show();

    }

    public int MouseListenerX() {
        return (int) StdDraw.mouseX();
    }

    public int MouseListenerY() {
